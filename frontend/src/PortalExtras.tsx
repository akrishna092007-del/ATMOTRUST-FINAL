...
import type { State, Task, User } from './types'
...
export function EmployeeTasks({ state, page }: { state: State; page: string }) {
  ...
  return <><div className="page-head"><div><p className="eyebrow">FIELD MAINTENANCE</p><h1>{page === 'history' ? 'Maintenance History' : page === 'details' ? 'Task Details' : 'Assigned Tasks'}</h1><p>Only tasks assigned to your station/pole are available.</p></div></div>{message && <p className="info" role="status">{message}</p>}{page !== 'details' && <section className="panel"><h3>{page === 'history' ? 'Completed work' : 'Current assignments'}</h3>{visible.length ? visible.map(item => <button className="task-row" key={item.incident_id} onClick={() => setSelectedId(item.incident_id)}><strong>{item.station_name} · {item.pole_id}</strong><span>{label(item.variable)} · {label(item.fault_type)} · {label(item.severity)}</span><b>{item.status}</b></button>) : <p className="empty">No tasks in this view.</p>}</section>}{task && page !== 'history' && <section className="panel task-detail"><div className="panel-head"><h3>{task.station_name} · {task.pole_id}</h3><span className="pill review">{task.status}</span></div><p><strong>{label(task.variable)} · probable {label(task.fault_type)} · {label(task.severity)}</strong></p><p>{task.explanation || 'Measured evidence is pending.'}</p><h4>Recommended resolution</h4><p>{task.resolution || 'Inspect the station and telemetry.'}</p><div className="detail-grid"><span>Trust Score <b>{task.trust_score ?? 'N/A'}</b></span><span>Confidence <b>{Math.round((task.confidence ?? 0) * 100)}%</b></span><span>Incident <b>{task.incident_id.slice(0, 8)}</b></span><span>Updated <b>{new Date(task.updated_at).toLocaleString()}</b></span></div><div className="button-row">{(task.status === 'Assigned' || task.status === 'Reopened') && <button className="primary" onClick={() => void change('Maintenance In Progress')}>Start Maintenance</button>}{task.status === 'Maintenance In Progress' && <button className="primary" onClick={() => void change('Maintained')}>Mark Maintained</button>}</div><h4>Maintenance notes</h4>{notes.map(note => <p className="note" key={note.id}><strong>{note.author}</strong> · {new Date(note.created_at).toLocaleString()}<br />{note.body}</p>)}<label>Add note<textarea value={draft} onChange={e => setDraft(e.target.value)} maxLength={2000} rows={3} /></label><button disabled={!draft.trim()} onClick={() => void addNote()}>Save note</button></section>}</>
}

export function AuthorityTasks({ state }: { state: State }) {
  const [tasks, setTasks] = useState<Task[]>([])
  const [users, setUsers] = useState<User[]>([])
  const [assignments, setAssignments] = useState<Assignment[]>([])
  const [message, setMessage] = useState('')
  useEffect(() => { void request<Task[]>('/tasks').then(setTasks).catch(error => setMessage(String(error))) }, [state.state_version])
  useEffect(() => { void Promise.all([request<User[]>('/admin/users'), request<Assignment[]>('/admin/assignments')]).then(([nextUsers, nextAssignments]) => { setUsers(nextUsers); setAssignments(nextAssignments) }).catch(error => setMessage(String(error))) }, [state.state_version])
