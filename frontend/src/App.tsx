...
import { AuthorityTasks, EmailAlerts, EmployeeTasks, ModelInformation, request, StationsEmployees } from './PortalExtras'
...
function StationMap({ state, selected, onSelect, onInvestigate }: { state: State; selected: string; onSelect: (id: string) => void; onInvestigate: () => void }) {
  ...
  return <section className="panel map-panel">
    <div className="panel-head"><h3>Station network</h3><span>{tiles ? 'Map tiles' : 'Offline coordinate layout'}</span></div>
...
function CommandCentre({ state, selected, select, navigate }: { state: State; selected: string; select: (id: string) => void; navigate: (page: string) => void }) {
  ...
  return <>
    ...
    <div className="summary-grid">
      {[['Monitored stations', state.stations.length], ['Likely faults', faults], ['Needs review', reviews], ['Open maintenance items', state.maintenance_items.length]].map(([name, value]) => <div className="summary-card" key={name}><span>{name}</span><strong>{value}</strong></div>)}
    </div>
...
function Investigation({ state, selected, setSelected, variable, setVariable, act }: { state: State; selected: string; setSelected: (v: string) => void; variable: Variable; setVariable: (v: Variable) => void; act: (id: string, action: string) => void }) {
  ...
  return <>
    <div className="page-head"><div><p className="eyebrow">SENSOR EVIDENCE</p><h1>Station Investigation</h1><p>Explain the current observation using causal and peer context.</p></div><label>Station <select value={selected} onChange={e => setSelected(e.target.value)}>{state.stations.map(s => <option key={s.station_id} value={s.station_id}>{s.name} · {s.station_id}</option>)}</select></label></div>
    ...
    <div className="metric-grid"><div className="metric"><span>Observed</span><strong>{valueText(observation?.value, units[variable])}</strong><small>{observation?.is_test_overlay ? 'Test overlay · ' : ''}{observation?.source_name ?? 'N/A'}</small></div><div className="metric"><span>Expected</span><strong>{valueText(assessment?.expected_value, units[variable])}</strong><small>Causal baseline</small></div><div className="metric"><span>Peer median</span><strong>{valueText(assessment?.peer_value, units[variable])}</strong><small>Nearby locations</small></div><div className="metric"><span>Sensor Health</span><strong>{health?.health_score ?? 'N/A'}</strong><small>{label(health?.health_status ?? 'unknown')}</small></div></div>
    <div className="two-col investigation-layout"><section className="panel"><ObservationChart state={state} variable={variable} selected={selected} investigation /><p className="muted">Source timestamp {shortTime(observation?.source_timestamp ?? null)} · Missing observations remain chart gaps. Expected and peer values come from backend assessments.</p></section><section className="panel trust-panel"><div className="panel-head"><h3>Observation assessment</h3><span>{label(assessment?.assessment_status ?? 'unavailable')}</span></div>{assessment ? <><div className="trust-number">{assessment.trust_score ?? 'N/A'}<small>/ 100</small></div><span className={`pill ${assessment.decision}`}>{label(assessment.decision)}</span><div className="detail-grid"><span>Confidence <b>{Math.round(assessment.confidence * 100)}%</b></span><span>Severity <b>{label(assessment.severity)}</b></span><span>Probable pattern <b>{label(assessment.fault_type)}</b></span><span>Trust band <b>{label(assessment.trust_band ?? 'N/A')}</b></span></div>{assessment.assessment_status === 'insufficient_context' && <p>Waiting for enough history or usable peers.</p>}</> : <p className="empty">Analysis unavailable.</p>}</section></div>
    <div className="two-col"><section className="panel"><div className="panel-head"><h3>Measured evidence</h3><span>Backend analysis</span></div><p>{assessment?.explanation}</p><div className="evidence-grid">{assessment?.evidence.length ? assessment.evidence.map((e, i) => <div className="evidence" key={`${e.kind}-${i}`}><span className={`evidence-sign ${e.support < 0 ? 'negative' : 'positive'}`}>{e.support < 0 ? '−' : '+'}</span><div><strong>{e.label}</strong><p>{e.detail}</p></div></div>) : <p className="empty">No evidence available.</p>}</div></section><section className="panel"><div className="panel-head"><h3>Operator review</h3><span>{incident ? label(incident.status) : 'No active incident'}</span></div><p>{assessment?.resolution || incident?.recommendation || 'No action required for the selected variable.'}</p><div className="button-row"><button disabled={!incident} onClick={() => incident && act(incident.incident_id, 'acknowledge')}>Acknowledge</button><button disabled={!incident} onClick={() => incident && act(incident.incident_id, 'confirm_genuine')}>Confirm Genuine</button><button disabled={!incident} onClick={() => incident && act(incident.incident_id, 'confirm_fault')}>Confirm Fault</button></div><p className="muted">Human review is recorded separately from the model assessment.</p></section></div>
  </>
}
