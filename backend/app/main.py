...
from .schema import Assessment, Command, Incident, InjectionSpec, now
...
async def lifespan(_app):
    ...
    try:
        ...
        if controller.clock and not controller.clock.done():
            controller.clock.cancel()
            try:
                await controller.clock
            except asyncio.CancelledError:
                pass
        if controller.email_tasks:
            await asyncio.gather(*list(controller.email_tasks), return_exceptions=True)
...
def history(run_id: str, limit: int = 60, user=Depends(current_user)):
    ...
    return {"run_id": run_id, "generation": controller.generation, "observations": rows[-limit * 15:]}
